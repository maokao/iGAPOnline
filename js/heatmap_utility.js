...
function changeColOrder(newOrder, heatmapId) {
    ...
    if (newOrder == "r2e" || newOrder == "singlelinkage" || newOrder == "averagelinkage" || newOrder == "completelinkage" || newOrder == "random") { 
        ...
        svg.select("#mv").selectAll(".rect")
            .attr("transform", function(d, i) {
                var col = parseInt(d3.select(this).attr("id"));
                return "translate(" + (sorted.indexOf(col) * cellWidth) + ",0)";
            });
        //if(svg.select("#mv3"))
        if(!isColProxfirst)
        {
        ...
        */
        svg.select("#mv").selectAll(".rect")
            .attr("transform", function(d, i) {
                var col = parseInt(d3.select(this).attr("id"));
                return "translate(" + (col * cellWidth) + ",0)";
            });
        if(!firstRunColTree)
            d3.selectAll("#colTree").remove();
        //if(svg.select("#mv3"))
...
function redrawHeatmap(nowID, x, y, mode, heatmapId) {
        ...
        if(mode == 0)
        {
            let max_data = d3.max(interval_min, function(row) { return d3.max(row) });
            let min_data = d3.min(interval_max, function(row) { return d3.min(row) });
            let colorScale;
            if(rdPaletteReverse)
            {
                colorScale = d3.scaleSequential()
                    .domain([min_data, max_data])
                    .interpolator(rd_colorID);  
            }
            else
            {
                colorScale = d3.scaleSequential()
            ...
            let cell = row.selectAll(".rect").selectAll(".cell")
            let update = cell.data(function(d) {
                    //j++;
                    //return d;
                    var interval_minmax = d.split(',');
                    let min = Number(interval_minmax[0]);
                    let max = Number(interval_minmax[1]);
                    var interval_data = [];
                    for (var i = 0; i < cellWidth; i++) {
                        interval_data.push((max-min)*i/cellWidth+min);
                    };
        ...
        if(mode == 0)
        {
            let t = row.selectAll(".rect")
                .attr("transform", function(d, i) {
                    var col = parseInt(d3.select(this).attr("id"));
                    if(colOrderId == "r2e")
                        return "translate(" + (col_r2e_order.indexOf(col) * cellWidth) + ",0)";
                    else if (colOrderId == "sortinit_col")
                        return "translate(" + (col * cellWidth) + ",0)";
                    else
                        return "translate(" + (col_output_order_array.indexOf(col) * cellWidth) + ",0)";
                });
...
function setupHeatmap2(nowdata, nowID, x, y, mode, heatmapId, colorID) {
        ...
        if(mode == 0)
        {
            max_data = d3.max(interval_min, function(row) { return d3.max(row) });
            min_data = d3.min(interval_max, function(row) { return d3.min(row) });
            rd_colorID = colorID;
        }
        else
        {
            max_data = d3.max(nowdata, function(row) { return d3.max(row) });
            min_data = d3.min(nowdata, function(row) { return d3.min(row) });            
        }
        ...
        if(mode == 0)
        {
            ...
            {
                temp_value = parseFloat(((max_data-min_data)/cellWidth)*i)+parseFloat(min_data);
                legend_text.push(temp_value);
            }

            var heatMap = row.selectAll(".rect")
            .data(function(d) {
                //j++;
                return d;
                //return legend_text;
            ...
            .on('click', function(d, i) {
                if (d != null) {
                    tooltip.html('<div class="heatmap_tooltip">' + data[Number(this.parentNode.parentNode.id)][Number(this.parentNode.id)] + '</div>');
